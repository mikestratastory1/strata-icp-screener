...
Evaluate whether the homepage hero and first 2 sections make a specific competitive claim that a competitor in the same category could NOT also make.

+1: Hero makes a specific competitive claim a competitor couldn't copy. The visitor immediately knows what's different.
Scoring examples for +1:
- Hero: "Web-scale search that's 100x cheaper, 10x faster" — specific metrics no competitor claims. Score 1.
- Hero: "The only AI security that's deterministic, explainable, and auditor-ready" — "the only" is an exclusionary claim. Score 1.
- Hero: "The First AI Platform Built for the Entire Revenue Journey" — "the first" stakes a category-creation claim. Score 1.

+2: Differentiators exist on the homepage but aren't in the hero, OR the hero makes a directionally correct claim using language a competitor could also use.
Scoring examples for +2:
- Hero: "Capture every customer interaction automatically. Sync it to your CRM" — could describe any CRM data tool. Real differentiators (time-travel data correction, 97% ML accuracy) appear in Section 15. Score 2.
- Hero: "Agentic-AI Platform to Scale Relationship Banking" with "boost productivity and grow margins" — directionally right but any banking AI tool could say this. Real value (predictive cross-sell, augmentation not replacement) appears later. Score 2.
- Hero: "Configuration as data lets you see what will happen before it happens" — strong hook but Section 3 benefits are generic and competitors could claim the same. Score 2.

+3: Homepage could belong to a different company in the same category. No specific competitive claim in the first 4 sections.
Scoring examples for +3:
- Hero: "Buying Intelligence for Enterprise Technology" with "Reviews, Discussions, and Advice from Real Users" — could be G2, Capterra, or any review platform. Score 3.
- Hero: "Software that moves. Apex.AI enables software-defined everything" — completely generic, no competitive claim. Score 3.
- Hero: "Response Management Solutions" with "Discover why organizations use Daupler" — could describe any incident management tool. Score 3.
- Hero: "THE FOOD YOU LOVE. THE PRICES YOU WANT" — generic food ordering, zero differentiation. Score 3.
- Hero: "The AI Powered CAD Tool" — could describe any AI CAD product. Score 3.
...
Evaluate whether the homepage leads with quantified business outcomes a buyer would recognize as their own KPIs, not product metrics or feature counts.
...
+1: Homepage leads with quantified business outcomes in hero or Section 2 tied to revenue, cost, risk, or time at a strategic level.
Scoring examples for +1:
- Hero: "60% Faster Go-Lives, 50% Lower Costs, 90% Fewer Errors" followed by "Turn Implementations Into A Revenue Accelerator" — quantified strategic outcomes above the fold. Score 1.
- Hero area includes "4x business goal achievement, 15% high performer density improvement, 42% grievance reduction" — specific buyer KPIs with numbers. Score 1.
- Hero: "Search More. Pay Less" with "100x cheaper, 10x faster" — quantified cost/performance outcomes. Score 1.

+2: Outcomes exist but are either unquantified in the hero, quantified but buried below Section 2, or framed as product metrics rather than buyer KPIs.
Scoring examples for +2:
- Hero says "accelerate time-to-market" without numbers, while case studies show "12+ month acceleration" — outcome exists but not quantified on homepage. Score 2.
- Quantified outcomes (20-75% callout reduction, $200K savings) exist but are in Section 7 requiring deep scroll — buried. Score 2.
- Hero: "Boost banker productivity and grow margins" — strategic intent but no numbers, no proof, overshadowed by feature sections below. Score 2.
- Homepage mentions "2-3 hours saved per rep" but it appears in a product feature section, not framed as a buyer outcome. Score 2.

+3: Homepage is feature-driven or mission-driven with no quantified outcomes in the first 4 sections.
Scoring examples for +3:
- Hero: "Building Frontier Open Intelligence" with no product details, no outcomes, no metrics anywhere visible — purely mission-driven. Score 3.
- Hero: "Simple, Elastic-Quality Search for Postgres" with feature toolkit (text search, hybrid search, boolean queries) — feature-driven. Case studies show 95% fewer timeouts and 25x faster queries but homepage doesn't surface them. Score 3.
- Hero: "THE FOOD YOU LOVE. THE PRICES YOU WANT" — no business outcomes, no metrics. Score 3.
- Hero lists product capabilities (secure agent authorization, tool governance) with no quantified business impact — feature-driven. Score 3.
...
First determine: has this company had a significant product, positioning, or go-to-market change in the last 12 months? Significant means: new product launch, major feature pivot, rebrand, new market entry, strategic partnership that changes positioning, or a fundamental shift in how the CEO describes the company. Incremental feature additions, infrastructure upgrades, and minor partnerships do NOT count.

Score 0: No significant change in the last 12 months. The company is executing on the same strategy with the same product narrative.
...
If a significant change DID occur, evaluate how well the homepage reflects the new direction:

+1: The homepage already reflects the new direction accurately.
Scoring examples for +1:
- Company launched a platform integration (Onshape) and homepage already positions as an integrated AI copilot rather than standalone tool. Score 1.
- Company added Salesforce integration and homepage already frames the product within the Salesforce ecosystem. Score 1.

+2: The homepage partially reflects the change. Elements of the new story appear but the old narrative still dominates.
Scoring examples for +2:
- Company shifted from creator tool to enterprise platform. Homepage lists the enterprise product as a feature but still frames the company as a creator clipping tool. Score 2.
- Company expanded from single-utility focus to multi-utility. Homepage shows multi-utility support but doesn't tell the expansion story. Score 2.
- Company launched AI agents. Homepage mentions them but still frames the company as a workflow automation tool. Score 2.

+3: The homepage doesn't reflect the change at all. A visitor gets a materially different picture than reality.
Scoring examples for +3:
- Company pivoted from coding agent to foundation models. Homepage still says "Building Frontier Open Intelligence" with no product details. Score 3.
- Company launched a compliance automation product (PetComply.ai). Homepage still shows old pet screening positioning with no mention of the new product. Score 3.
- Company shifted from data cleanup to AI-driven GTM outcomes. Homepage still leads with "Build Agentic GTM Systems" and CRM data capture. Score 3.
- Company repositioned from search extension to rethinking Postgres internals. Homepage still says "Elasticsearch alternative." Score 3.
...
Empty array [] for changes if no significant product changes. Score 0 if no changes found.`;
...
export default function ICPScreener() {
  ...
  const DB_ACCOUNT_FILTER_CATALOG = [
    ...
    { key: 'score_a', label: 'Factor A: Differentiation', category: 'Gap Scores', inputType: 'number', operators: ['=', '>', '<', '>=', '<='], defaultOp: '>=' },
    { key: 'score_b', label: 'Factor B: Outcomes', category: 'Gap Scores', inputType: 'number', operators: ['=', '>', '<', '>=', '<='], defaultOp: '>=' },
    { key: 'score_c', label: 'Factor C: Customer-centric', category: 'Gap Scores', inputType: 'number', operators: ['=', '>', '<', '>=', '<='], defaultOp: '>=' },
    { key: 'score_d', label: 'Factor D: Product Change', category: 'Gap Scores', inputType: 'number', operators: ['=', '>', '<', '>=', '<='], defaultOp: '>=' },
    { key: 'score_e', label: 'Factor E: Audience Change', category: 'Gap Scores', inputType: 'number', operators: ['=', '>', '<', '>=', '<='], defaultOp: '>=' },
    { key: 'score_f', label: 'Factor F: Multi-product', category: 'Gap Scores', inputType: 'number', operators: ['=', '>', '<', '>=', '<='], defaultOp: '>=' },
    { key: 'score_g', label: 'Factor G: Vision Gap', category: 'Gap Scores', inputType: 'number', operators: ['=', '>', '<', '>=', '<='], defaultOp: '>=' },
  ...
  const processCompany = async (index) => {
    ...
    try {
      ...
      const fieldsToCheck = [
        ...
        { key: 'top3Outcomes', label: 'Top 3 outcomes' },
        { key: 'top3Differentiators', label: 'Top 3 differentiators' },
      ...
      for (const f of fieldsToCheck) {
        ...
        const isEmpty = !val || val.trim().length < 5 || 
          ...
          val.toLowerCase() === 'n/a';
        if (isEmpty) warnings.push(`${f.label}: empty or not found`);
        else if (val.trim().length < 30) warnings.push(`${f.label}: thin (${val.trim().length} chars)`);
      }
      updateCompany({ screeningWarnings: warnings });
      ...
      // Factor D score 0 means no change detected — exclude from total and gap selection
      const scorableFactors = Object.entries(scores).filter(([k, v]) => !(k === 'D' && v === 0));
              ...
              {checkedAccounts.size > 0 && (
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
                  ...
                  <button onClick={async () => {
                    ...
                    addLog(`Re-screening ${toRescreen.length} accounts...`);
                    for (const co of toRescreen) {
                      const idx = companies.findIndex(c => c.domain === co.domain);
                      if (idx >= 0) {
                        setCompanies(prev => { const u = [...prev]; u[idx] = { ...u[idx], status: 'pending', step: 'Queued for re-screen...' }; return u; });
                        await processCompany(idx);
                        await new Promise(r => setTimeout(r, 1000));
                      }
                    }
                    addLog(`Re-screen complete for ${toRescreen.length} accounts`);
                    ...
                    {checkedStaged.size > 0 && (
                      <>
                        <button onClick={async () => {
                          ...
                          // Navigate to Discover Contacts with domains pre-filled
                          ...
                          Find Contacts & Add to Database ({checkedStaged.size})
                        </button>
                        ...
                        <button onClick={async () => {
                          ...
                          for (let i = 0; i < domains.length; i++) {
                            // Find index from current state snapshot each time
                            let idx = -1;
                            setCompanies(prev => {
                              idx = prev.findIndex(c => c.domain === domains[i]);
                              if (idx >= 0) {
                                const u = [...prev];
                                u[idx] = { ...u[idx], dbRunId: null, status: 'pending', step: `Queued (${i+1}/${domains.length})` };
                                return u;
                              }
                              return prev;
                            });
