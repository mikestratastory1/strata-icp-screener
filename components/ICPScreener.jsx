...
export default function ICPScreener() {
  ...
  useEffect(() => {
    if (supabase) { getSavedFilters().then(all => { setSavedFilters(all.filter(f => f.mode !== 'people')); setSavedPeopleFilters(all.filter(f => f.mode === 'people')); }).catch(() => {}); }
    if (supabase) { getInstantlyActivity().then(rows => {
      const activity = {};
      for (const r of rows) { activity[r.contact_email.toLowerCase()] = { sent: r.emails_sent || 0, replies: r.replies || 0, lastSyncedAt: r.last_synced_at }; }
      setInstantlyActivity(activity);
      if (rows.length > 0) addLog(`Loaded activity for ${rows.length} contacts from database`);
    }).catch(() => {}); }
  }, []);
  ...
  const [peopleFilterGroupOp, setPeopleFilterGroupOp] = useState('and'); // 'and' or 'or' between filter groups
  const [peopleFilterGroups, setPeopleFilterGroups] = useState([{ id: 1, op: 'and', filterIds: [] }]); // groups of filter IDs
  const peopleGroupIdCounter = useRef(1);
  const [savedPeopleFilters, setSavedPeopleFilters] = useState([]);
  const [showSavedFilterPicker, setShowSavedFilterPicker] = useState(false);
  const [savePeopleFilterName, setSavePeopleFilterName] = useState('');
  const [showSaveFilterInput, setShowSaveFilterInput] = useState(false);
  const [activeFilterGroupId, setActiveFilterGroupId] = useState(1); // which group new filters get added to

  const addPeopleFilter = (fieldKey, groupId = null) => {
    ...
    const targetGroup = groupId || activeFilterGroupId;
    setPeopleFilterGroups(prev => prev.map(g => g.id === targetGroup ? { ...g, filterIds: [...g.filterIds, id] } : g));
    setShowPeopleFilterPicker(false);
  };

  const addPeopleFilterGroup = () => {
    const newId = ++peopleGroupIdCounter.current;
    setPeopleFilterGroups(prev => [...prev, { id: newId, op: 'and', filterIds: [] }]);
    setActiveFilterGroupId(newId);
  };

  const removePeopleFilterGroup = (groupId) => {
    setPeopleFilterGroups(prev => {
      const group = prev.find(g => g.id === groupId);
      if (group) {
        // Also remove the filters in this group
        setPeopleFilters(pf => pf.filter(f => !group.filterIds.includes(f.id)));
      }
      const remaining = prev.filter(g => g.id !== groupId);
      if (remaining.length === 0) {
        const newId = ++peopleGroupIdCounter.current;
        setActiveFilterGroupId(newId);
        return [{ id: newId, op: 'and', filterIds: [] }];
      }
      if (activeFilterGroupId === groupId) setActiveFilterGroupId(remaining[0].id);
      return remaining;
    });
  };

  const savePeopleFilter = async () => {
    if (!savePeopleFilterName.trim()) return;
    try {
      const filterConfig = { filters: peopleFilters, groups: peopleFilterGroups, groupOp: peopleFilterGroupOp };
      const saved = await createSavedFilter(savePeopleFilterName.trim(), 'people', filterConfig);
      setSavedPeopleFilters(prev => [saved, ...prev]);
      setSavePeopleFilterName('');
      setShowSaveFilterInput(false);
      addLog(`Saved filter: ${savePeopleFilterName.trim()}`);
    } catch (err) { addLog(`Save filter error: ${err.message}`); }
  };

  const loadPeopleFilter = (saved) => {
    const config = saved.filters;
    if (!config) return;
    setPeopleFilters(config.filters || []);
    setPeopleFilterGroups(config.groups || [{ id: 1, op: 'and', filterIds: [] }]);
    setPeopleFilterGroupOp(config.groupOp || 'and');
    // Update counters
    const maxFilterId = Math.max(0, ...(config.filters || []).map(f => f.id));
    const maxGroupId = Math.max(0, ...(config.groups || []).map(g => g.id));
    peopleFilterIdCounter.current = maxFilterId;
    peopleGroupIdCounter.current = maxGroupId;
    if (config.groups?.length > 0) setActiveFilterGroupId(config.groups[0].id);
    setShowSavedFilterPicker(false);
    addLog(`Loaded filter: ${saved.name}`);
  };
  ...
  const removePeopleFilter = (id) => {
    ...
    setPeopleFilterGroups(prev => prev.map(g => ({ ...g, filterIds: g.filterIds.filter(fid => fid !== id) })));
  ...
  // Build filters from PEOPLE_FILTER_CATALOG dynamic filters with group logic
  const buildDynamicPeopleFilters = () => {
    ...
    if (Array.isArray(contactsCompany)) {
      ...
      if (domains.length === 1) {
        topConditions.push({ column: 'current_employers.company_website_domain', type: '=', value: domains[0] });
      } else {
        topConditions.push({ column: 'current_employers.company_website_domain', type: 'in', value: domains });
      }
    } else {
      if (!contactsCompany.domain) return null;
      topConditions.push({ column: 'current_employers.company_website_domain', type: '=', value: contactsCompany.domain });
    }

    // Build conditions per group
    const buildConditionsForFilters = (filterIds) => {
      const conditions = [];
      for (const fId of filterIds) {
        const f = peopleFilters.find(pf => pf.id === fId);
        if (!f) continue;
    ...
    for (const group of peopleFilterGroups) {
      if (group.filterIds.length === 0) continue;
      const conditions = buildConditionsForFilters(group.filterIds);
      if (conditions.length === 0) continue;
      if (conditions.length === 1) {
        groupConditions.push(conditions[0]);
      } else {
        groupConditions.push({ op: group.op, conditions });
      }
    }

    // Also handle any ungrouped filters (backward compatibility)
    const allGroupedIds = new Set(peopleFilterGroups.flatMap(g => g.filterIds));
    const ungroupedFilters = peopleFilters.filter(f => !allGroupedIds.has(f.id));
    if (ungroupedFilters.length > 0) {
      const conditions = buildConditionsForFilters(ungroupedFilters.map(f => f.id));
      groupConditions.push(...conditions);
    }

    // Combine groups with the inter-group operator
    if (groupConditions.length > 0) {
      if (groupConditions.length === 1) {
        topConditions.push(groupConditions[0]);
      } else {
        topConditions.push({ op: peopleFilterGroupOp, conditions: groupConditions });
      }
    }

    if (topConditions.length === 0) return null;
    return { op: 'and', conditions: topConditions };
  };
  ...
  const DB_ACCOUNT_FILTER_CATALOG = [
    ...
    { key: 'data_gaps', label: 'Missing Fields', category: 'Status', inputType: 'number', operators: ['=', '>', '<', '>=', '<='], defaultOp: '>' },
    { key: 'missing_field', label: 'Missing Field', category: 'Status', inputType: 'multi_select', operators: ['in'], defaultOp: 'in', options: ['Product summary', 'Target customer', 'Decision maker', 'Outcomes', 'Differentiators', 'Competitors', 'Homepage sections', 'CEO narrative', 'CEO content', 'Announcements', 'Case studies'] },
  ...
  const DB_ACCOUNT_COLUMNS = [
    ...
    { key: 'data_gaps', label: 'Data Gaps', default: false },
  ...
  const DB_CONTACT_COLUMNS = [
    ...
    { key: 'outcomes', label: 'Outcomes', default: false },
    { key: 'competitors', label: 'Competitors', default: false },
    { key: 'campaigns', label: 'Campaigns', default: false },
    { key: 'emails_sent', label: 'Emails Sent', default: false },
    { key: 'replies', label: 'Replies', default: false },
    { key: 'added', label: 'Date Added', default: false },
  ];
  ...
  const DB_CONTACT_FILTER_CATALOG = [
    ...
    { key: 'competitors', label: 'Competitors', category: 'Research', inputType: 'text', operators: ['contains'], defaultOp: 'contains' },
    { key: 'ceo_narrative', label: 'CEO Narrative', category: 'Research', inputType: 'text', operators: ['contains'], defaultOp: 'contains' },
    { key: 'emails_sent', label: 'Emails Sent', category: 'Activity', inputType: 'number', operators: ['=', '>', '<', '>=', '<='], defaultOp: '>=' },
    { key: 'replies', label: 'Replies', category: 'Activity', inputType: 'number', operators: ['=', '>', '<', '>=', '<='], defaultOp: '>=' },
    { key: 'has_replied', label: 'Has Replied', category: 'Activity', inputType: 'boolean', operators: ['='], defaultOp: '=', defaultValue: true },
  ];
      ...
      try {
        ...
        if (rows && rows.length > 0) {
          const loaded = rows.map(r => {
            ...
            // Compute warnings and data gaps from loaded data
            if (co.status === 'complete' && r.run_id) {
              const w = [];
              const missing = [];
              const checkField = (val, label, minLen = 5) => {
                const v = (val || '').trim();
                const isEmpty = !v || v.length < minLen || 
                  v.toLowerCase().includes('not found') || 
                  v.toLowerCase().includes('none found') || 
                  v.toLowerCase().includes('not available') ||
                  v.toLowerCase() === 'n/a';
                if (isEmpty) { w.push(`${label}: missing`); missing.push(label); }
              };
              checkField(r.product_summary, 'Product summary');
              checkField(r.target_customer, 'Target customer');
              checkField(r.target_decision_maker, 'Decision maker');
              checkField(r.top3_outcomes, 'Outcomes');
              checkField(r.top3_differentiators, 'Differentiators');
              checkField(r.competitors, 'Competitors');
              checkField(r.homepage_sections, 'Homepage sections', 100);
              checkField(r.ceo_narrative_theme, 'CEO narrative');
              checkField(r.ceo_recent_content, 'CEO content');
              checkField(r.major_announcements, 'Announcements');
              checkField(r.case_study_customers, 'Case studies');
              if (co.totalScore === 0 && co.icpFit !== 'Disqualified') w.push('All scoring data missing — re-screen recommended');
              co.screeningWarnings = w;
              co.missingFields = missing;
              co.dataCompleteness = Math.round(((11 - missing.length) / 11) * 100);
            }
            return co;
          });
          setCompanies(loaded);
...
=== END CEO CONTENT ===`;
  ...
  const loadCampaignData = async (campaignId) => {
    ...
    try {
      ...
      // Auto-sync Instantly activity if campaign is linked
      if (camp?.instantly_campaign_id) {
        syncInstantlyActivity(camp.instantly_campaign_id);
      }
    } catch (err) {
  ...
  const [instantlyActivity, setInstantlyActivity] = useState({}); // { contactEmail: { sent: N, replies: N, lastSyncedAt } }
  const [instantlySyncing, setInstantlySyncing] = useState(false);

  const syncInstantlyActivityInner = async (instantlyCampaignId) => {
    addLog(`[instantly] Fetching emails for campaign ${instantlyCampaignId}...`);
    try {
      let allEmails = [];
      let cursor = null;
      for (let page = 0; page < 20; page++) {
        const params = { action: 'list_emails', campaign_id: instantlyCampaignId, limit: 100 };
        if (cursor) params.starting_after = cursor;
        const resp = await fetch('/api/instantly', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(params) });
        const data = await resp.json();
        if (!resp.ok) { addLog(`[instantly] API error (${resp.status}): ${JSON.stringify(data.error || data)}`); break; }
        if (!data.items) { addLog(`[instantly] No items in response. Keys: ${Object.keys(data).join(', ')}. Full: ${JSON.stringify(data).substring(0, 200)}`); break; }
        allEmails = allEmails.concat(data.items);
        addLog(`[instantly] Page ${page + 1}: ${data.items.length} emails (${allEmails.length} total)`);
        if (!data.next_starting_after || data.items.length < 100) break;
        cursor = data.next_starting_after;
      }

      if (allEmails.length === 0) {
        addLog(`[instantly] No emails found for campaign ${instantlyCampaignId}`);
        return;
      }

      // Log ue_type distribution for debugging
      const typeCounts = {};
      for (const e of allEmails) { typeCounts[e.ue_type] = (typeCounts[e.ue_type] || 0) + 1; }
      addLog(`[instantly] Email types: ${JSON.stringify(typeCounts)} (1=sent, 2=received, 3=manual, 4=scheduled)`);

      const sentEmails = allEmails.filter(e => e.ue_type === 1);
      const receivedEmails = allEmails.filter(e => e.ue_type === 2);

      const activity = {};
      for (const e of sentEmails) {
        const lead = (e.lead || '').toLowerCase();
        if (!lead) continue;
        if (!activity[lead]) activity[lead] = { sent: 0, replies: 0 };
        activity[lead].sent++;
      }
      for (const e of receivedEmails) {
        ...
        if (!lead) continue;
        if (!activity[lead]) activity[lead] = { sent: 0, replies: 0 };
        activity[lead].replies++;
      }

      const now = new Date().toISOString();
      for (const email of Object.keys(activity)) {
        activity[email].lastSyncedAt = now;
      }

      setInstantlyActivity(prev => ({ ...prev, ...activity }));
      addLog(`[instantly] Synced: ${allEmails.length} total (${sentEmails.length} sent, ${receivedEmails.length} replies) across ${Object.keys(activity).length} leads`);

      try {
        const rows = Object.entries(activity).map(([email, a]) => {
          const contact = allContacts.find(c => (c.business_email || '').toLowerCase() === email);
          return {
            contact_email: email,
        ...
        await upsertInstantlyActivity(rows);
        addLog(`[instantly] Persisted activity for ${rows.length} contacts`);
      } catch (err) {
        addLog(`[instantly] Persist error: ${err.message}`);
      }
    } catch (err) {
      addLog(`[instantly] Sync error: ${err.message}`);
    }
  };

  const syncInstantlyActivity = async (instantlyCampaignId) => {
    if (!instantlyCampaignId || instantlySyncing) return;
    setInstantlySyncing(true);
    await syncInstantlyActivityInner(instantlyCampaignId);
    setInstantlySyncing(false);
  };

  const syncAllInstantlyActivity = async () => {
    const linkedCampaigns = campaigns.filter(c => c.instantly_campaign_id);
    if (linkedCampaigns.length === 0) { addLog('[instantly] No campaigns linked to Instantly. Link a campaign in the Campaigns tab first.'); return; }
    if (instantlySyncing) { addLog('[instantly] Sync already in progress'); return; }
    setInstantlySyncing(true);
    addLog(`[instantly] Syncing across ${linkedCampaigns.length} linked campaign(s)...`);
    for (const camp of linkedCampaigns) {
      addLog(`[instantly] Campaign: "${camp.name}"`);
      await syncInstantlyActivityInner(camp.instantly_campaign_id);
    }
    setInstantlySyncing(false);
    addLog(`[instantly] Done syncing all campaigns`);
  };
  ...
  const pushToInstantly = async (instantlyCampaignId, filterContactIds = null) => {
    ...
    try {
      ...
      // Auto-activate the Instantly campaign if it's completed/paused
      try {
        addLog(`[instantly] Activating campaign ${instantlyCampaignId}...`);
        await fetch('/api/instantly', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'activate_campaign', campaign_id: instantlyCampaignId }) });
      } catch (err) {
        addLog(`[instantly] Activate warning (non-fatal): ${err.message}`);
      }
              ...
              {checkedAccounts.size > 0 && (
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
                  ...
                  <button onClick={async () => {
                    ...
                    setCompanies(prev => prev.map(c => domains.includes(c.domain) ? { ...c, dbRunId: null, status: 'pending', step: 'Queued for re-screen' } : c));
                    addLog(`Added ${domains.length} accounts to Screen Queue for re-screening`);
                    setCheckedAccounts(new Set());
            ...
            <div className={`${selectedCompany !== null ? 'w-1/4 min-w-[220px]' : 'w-full'} flex flex-col`}>
              {(() => {
                ...
                for (const f of dbAccountFilters) {
                  ...
                  } else if (spec.key === 'data_gaps') {
                    if (f.value !== '') {
                      const val = parseFloat(f.value);
                      if (!isNaN(val)) {
                        filtered = filtered.filter(c => {
                          const gaps = (c.missingFields || []).length;
                          return f.operator === '=' ? gaps === val : f.operator === '>' ? gaps > val : f.operator === '<' ? gaps < val : f.operator === '>=' ? gaps >= val : f.operator === '<=' ? gaps <= val : true;
                        });
                      }
                    }
                  } else if (spec.key === 'missing_field') {
                    if (Array.isArray(f.value) && f.value.length > 0) {
                      filtered = filtered.filter(c => f.value.some(mf => (c.missingFields || []).includes(mf)));
                    }
                  }
                }
                ...
                const scoreMap = (c, k) => ({ score: c.totalScore, name: c.companyName, icp_fit: c.icpFit, status: c.accountStatus || 'Cold', contacts: getContactCount(c), gap1: c.gap1Factor, gap2: c.gap2Factor, score_a: c.scoreA, score_b: c.scoreB, score_c: c.scoreC, score_d: c.scoreD, score_e: c.scoreE, score_f: c.scoreF, score_g: c.scoreG, d_announcement: c.dAnnouncementDate, a_verdict: c.aVerdict, b_verdict: c.bVerdict, d_verdict: c.dVerdict, g_verdict: c.gVerdict, decision_maker: c.targetDecisionMaker || c.bDecisionMaker, differentiators: c.aDifferentiators, outcomes: c.bOutcomes, competitors: c.competitors, ceo_narrative: c.ceoNarrativeTheme, funding: c.funding, data_gaps: (c.missingFields || []).length, screened: c.lastScreenedAt, added: c.addedAt, newest: c.addedAt }[k]);
                    ...
                    <table className="w-full text-sm">
                      <thead className="sticky top-0 bg-white border-b border-gray-200 z-10">
                        <tr>
                          ...
                          {accountVisibleCols.includes('data_gaps') && <SortHeader col="data_gaps" label="Data Gaps" />}
                        ...
                        {filtered.length > 0 ? filtered.map((c) => {
                          ...
                          return (
                            <tr key={c.domain || origIdx} className={`cursor-pointer transition-colors ${isChecked ? 'bg-violet-50' : isSelected ? 'bg-violet-50' : 'hover:bg-gray-50'}`}>
                              ...
                              {accountVisibleCols.includes('data_gaps') && <td className="px-2 py-2" onClick={() => setSelectedCompany(isSelected ? null : origIdx)}>
                                {c.missingFields && c.missingFields.length > 0 ? (
                                  <span className={`text-[9px] px-1.5 py-0.5 rounded ${c.missingFields.length >= 5 ? 'bg-red-50 text-red-600' : c.missingFields.length >= 3 ? 'bg-amber-50 text-amber-600' : 'bg-gray-50 text-gray-500'}`} title={c.missingFields.join(', ')}>{c.missingFields.length} missing</span>
                                ) : c.status === 'complete' ? (
                                  <span className="text-[9px] text-green-500">Complete</span>
                                ) : null}
                              </td>}
          ...
          <div>
            ...
            <div className="mb-3 space-y-2">
              <div className="flex items-center gap-3 text-sm flex-wrap">
                ...
                <button onClick={syncAllInstantlyActivity} disabled={instantlySyncing}
                  className="px-2.5 py-1 bg-gray-50 hover:bg-gray-100 text-gray-500 rounded text-[10px] border border-gray-200 disabled:opacity-50">
                  {instantlySyncing ? 'Syncing...' : 'Sync Instantly'}
                </button>
            ...
            {/* Bulk actions bar — visible when contacts are checked */}
            {checkedContacts.size > 0 && (
              <div className="mb-3 flex items-center gap-2 bg-violet-50 border border-violet-200 rounded-lg px-4 py-2">
                <span className="text-violet-700 text-xs font-medium">{checkedContacts.size} selected</span>
                <div className="flex items-center gap-2 ml-4">
                  <div className="relative">
                    <button onClick={() => setBulkCampaignPicker(!bulkCampaignPicker)} className="px-3 py-1.5 bg-violet-600 hover:bg-violet-500 text-white rounded-lg text-xs font-medium">+ Add to Campaign</button>
                    {bulkCampaignPicker && (
                      <div className="absolute left-0 top-8 z-50 bg-white border border-gray-200 rounded-lg shadow-xl py-1 min-w-[200px]">
                        {campaigns.length > 0 ? campaigns.map(c => (
                          ...
                          <div className="px-4 py-2 text-xs text-gray-500">No campaigns yet. Create one in the Campaigns tab.</div>
                        )}
                      </div>
                    )}
                  </div>
                  <button onClick={async () => {
                    ...
                    addLog(`Email enrichment complete.`);
                    setCheckedContacts(new Set());
                  }} className="px-3 py-1.5 text-emerald-600 hover:text-emerald-700 border border-emerald-200 hover:bg-emerald-50 bg-white rounded-lg text-xs font-medium">Find Emails</button>
                  <button onClick={handleBulkDeleteContacts} className="px-3 py-1.5 text-red-500 hover:text-red-700 border border-red-200 hover:bg-red-50 bg-white rounded-lg text-xs font-medium">Delete</button>
                </div>
                <button onClick={() => setCheckedContacts(new Set())} className="ml-auto text-xs text-violet-500 hover:text-violet-700">Clear</button>
              </div>
            )}
            {/* Split panel */}
            <div className="flex gap-4" style={{ minHeight: '70vh' }}>
            ...
            <div className={`${selectedContactId ? 'w-1/3 min-w-[280px]' : 'w-full'} flex flex-col`}>
              {(() => {
                ...
                for (const f of dbContactFilters) {
                  ...
                  } else if (spec.key === 'emails_sent' || spec.key === 'replies') {
                    const val = parseFloat(f.value);
                    if (!isNaN(val)) {
                      filtered = filtered.filter(c => {
                        const a = instantlyActivity[(c.business_email || '').toLowerCase()];
                        const count = spec.key === 'emails_sent' ? (a?.sent || 0) : (a?.replies || 0);
                        return f.operator === '=' ? count === val : f.operator === '>' ? count > val : f.operator === '<' ? count < val : f.operator === '>=' ? count >= val : f.operator === '<=' ? count <= val : true;
                      });
                    }
                  } else if (spec.key === 'has_replied') {
                    filtered = filtered.filter(c => {
                      const a = instantlyActivity[(c.business_email || '').toLowerCase()];
                      return (a?.replies || 0) > 0;
                    });
                  }
                }
                // Sort
                ...
                const contactSortVal = (ct, k) => {
                  ...
                  const activity = instantlyActivity[(ct.business_email || '').toLowerCase()];
                  return { name: ct.name, title: ct.title, company: ct.companies?.name || ct.company_domain, seniority: ct.seniority, function: ct.function_category, status: ct.contact_status || 'New', email: ct.business_email ? 1 : 0, score: acct?.totalScore || 0, icp_fit: acct?.icpFit || '', gap1: acct?.gap1Factor || '', gap2: acct?.gap2Factor || '', score_a: acct?.scoreA || 0, score_b: acct?.scoreB || 0, score_c: acct?.scoreC || 0, score_d: acct?.scoreD || 0, score_e: acct?.scoreE || 0, score_f: acct?.scoreF || 0, score_g: acct?.scoreG || 0, emails_sent: activity?.sent || 0, replies: activity?.replies || 0, campaigns: allCampaignContacts.filter(cc => cc.contact_id === ct.id).length, newest: ct.created_at, added: ct.created_at }[k];
                };
                filtered.sort((a, b) => {
                    ...
                    <table className="w-full text-sm">
                      <thead className="sticky top-0 bg-white border-b border-gray-200 z-10">
                        <tr>
                          ...
                          {contactVisibleCols.includes('campaigns') && <CSortHeader col="campaigns" label="Campaigns" />}
                          {contactVisibleCols.includes('emails_sent') && <CSortHeader col="emails_sent" label="Sent" />}
                          {contactVisibleCols.includes('replies') && <CSortHeader col="replies" label="Replies" />}
                        ...
                        {filtered.length > 0 ? filtered.map(ct => {
                          ...
                          return (
                            <tr key={ct.id} className={`cursor-pointer transition-colors ${isChecked ? 'bg-violet-50' : isSelected ? 'bg-rose-50' : 'hover:bg-gray-50'}`}>
                              ...
                              {contactVisibleCols.includes('campaigns') && <td className="px-2 py-2" onClick={() => setSelectedContactId(isSelected ? null : ct.id)}>
                                {(() => {
                                  const contactCampaigns = allCampaignContacts
                                    .filter(cc => cc.contact_id === ct.id)
                                    .map(cc => campaigns.find(camp => camp.id === cc.campaign_id))
                                    .filter(Boolean);
                                  return contactCampaigns.length > 0 ? (
                                    <div className="flex flex-wrap gap-0.5">
                                      {contactCampaigns.map(camp => (
                                        <span key={camp.id} className="text-[9px] bg-violet-50 text-violet-600 px-1.5 py-0.5 rounded border border-violet-200 whitespace-nowrap">{camp.name}</span>
                                      ))}
                                    </div>
                                  ) : <span className="text-[9px] text-gray-300">None</span>;
                                })()}
                              </td>}
                              {contactVisibleCols.includes('emails_sent') && <td className="px-2 py-2 text-center" onClick={() => setSelectedContactId(isSelected ? null : ct.id)}>
                                {(() => { const a = instantlyActivity[(ct.business_email || '').toLowerCase()]; return a?.sent ? <span className="text-[10px] text-gray-600 font-medium">{a.sent}</span> : <span className="text-[10px] text-gray-300">0</span>; })()}
                              </td>}
                              {contactVisibleCols.includes('replies') && <td className="px-2 py-2 text-center" onClick={() => setSelectedContactId(isSelected ? null : ct.id)}>
                                {(() => { const a = instantlyActivity[(ct.business_email || '').toLowerCase()]; return a?.replies ? <span className="text-[10px] text-green-600 font-medium">{a.replies}</span> : <span className="text-[10px] text-gray-300">0</span>; })()}
                              </td>}
                  ...
                  {(() => {
                    ...
                    return linkedId ? (
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        <button onClick={() => {
                          const activeCamp = campaigns.find(c => c.id === activeCampaignId);
                          if (activeCamp?.instantly_campaign_id) syncInstantlyActivity(activeCamp.instantly_campaign_id);
                        }} disabled={instantlySyncing}
                          className="text-[10px] text-violet-500 hover:text-violet-700 disabled:text-gray-300">
                          {instantlySyncing ? 'Syncing...' : 'Sync Activity'}
                        </button>
                        <button onClick={async () => {
                          try {
                  ...
                  <div className="px-4 py-3 border-b border-gray-100">
                    ...
                    {checkedCampaignContacts.size > 0 && (
                      <div className="mt-2 flex flex-col gap-1">
                        ...
                        }} className="w-full px-2 py-1 bg-violet-600 text-white rounded text-[9px] font-medium text-center">Generate {checkedCampaignContacts.size}</button>
                        <button onClick={() => {
                          const activeCamp = campaigns.find(c => c.id === activeCampaignId);
                          if (activeCamp?.instantly_campaign_id) {
                        ...
                        }} className="w-full px-2 py-1 bg-gray-100 text-gray-600 rounded text-[9px] font-medium border border-gray-200 text-center">Push {checkedCampaignContacts.size}</button>
                        <button onClick={async () => {
                          const ids = [...checkedCampaignContacts];
                          try {
                            await removeCampaignContacts(activeCampaignId, ids);
                            setCampaignContacts(prev => prev.filter(cc => !checkedCampaignContacts.has(cc.contact_id)));
                            setCheckedCampaignContacts(new Set());
                            addLog(`Removed ${ids.length} contacts from campaign`);
                          } catch (err) { addLog(`Remove error: ${err.message}`); }
                        }} className="w-full px-2 py-1 bg-red-50 text-red-600 rounded text-[9px] font-medium border border-red-200 text-center">Remove {checkedCampaignContacts.size}</button>
                        <button onClick={() => setCheckedCampaignContacts(new Set())} className="w-full px-2 py-0.5 text-gray-400 text-[9px] text-center">Clear selection</button>
                      </div>
                    )}
                  </div>
                          ...
                          {(ec.components || []).sort((a, b) => a.component_order - b.component_order).map((comp, compIdx) => {
                            ...
                            <div key={comp.id} className="flex items-start gap-2 bg-gray-50 rounded-lg px-3 py-2">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {/* Inline editor — only for text components, not prompt components (use Design UI instead) */}
                                {compType !== 'prompt' && (
                                <details open={isDirty}>
                                  ...
                                  }} rows={3} className="w-full mt-1 bg-white border border-gray-200 rounded px-2 py-1 text-[10px] text-gray-600 font-mono leading-relaxed" />
        ...
        {activeView === 'discover_contacts' && (
          <div className="bg-white border border-gray-200 rounded-lg flex flex-col" style={{ height: '85vh' }}>
              ...
              <div className="px-6 py-4 border-b border-gray-100 space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <div className="relative">
                    <button onClick={() => setShowSavedFilterPicker(!showSavedFilterPicker)}
                      className="px-2.5 py-1 bg-gray-50 hover:bg-gray-100 text-gray-600 rounded text-[10px] border border-gray-200">
                      {savedPeopleFilters.length > 0 ? `Saved Filters (${savedPeopleFilters.length})` : 'Saved Filters'}
                    </button>
                    {showSavedFilterPicker && (
                      <div className="absolute left-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl z-50 min-w-[220px] max-h-[300px] overflow-auto">
                        {savedPeopleFilters.length > 0 ? savedPeopleFilters.map(sf => (
                          <div key={sf.id} className="flex items-center justify-between px-3 py-2 hover:bg-violet-50 group">
                            <button onClick={() => loadPeopleFilter(sf)} className="text-xs text-gray-700 hover:text-violet-700 flex-1 text-left">{sf.name}</button>
                            <button onClick={async (e) => { e.stopPropagation(); await deleteSavedFilter(sf.id); setSavedPeopleFilters(prev => prev.filter(x => x.id !== sf.id)); addLog(`Deleted filter: ${sf.name}`); }}
                              className="text-gray-300 hover:text-red-500 text-[10px] opacity-0 group-hover:opacity-100 ml-2">×</button>
                          </div>
                        )) : (
                          <div className="px-3 py-2 text-[10px] text-gray-400">No saved filters yet.</div>
                        )}
                      </div>
                    )}
                  </div>
                    ...
                    <>
                      {showSaveFilterInput ? (
                        <div className="flex items-center gap-1">
                          <input type="text" value={savePeopleFilterName} onChange={e => setSavePeopleFilterName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') savePeopleFilter(); }}
                            placeholder="Filter name..." className="bg-white border border-gray-200 rounded px-2 py-1 text-[10px] text-gray-700 w-32" autoFocus />
                          <button onClick={savePeopleFilter} className="px-2 py-1 bg-violet-600 text-white rounded text-[10px]">Save</button>
                          <button onClick={() => { setShowSaveFilterInput(false); setSavePeopleFilterName(''); }} className="text-gray-400 text-[10px]">Cancel</button>
                        </div>
                      ) : (
                        <button onClick={() => setShowSaveFilterInput(true)} className="px-2.5 py-1 bg-violet-50 hover:bg-violet-100 text-violet-600 rounded text-[10px] border border-violet-200">Save Current</button>
                      )}
                      <button onClick={() => { setPeopleFilters([]); setPeopleFilterGroups([{ id: ++peopleGroupIdCounter.current, op: 'and', filterIds: [] }]); setActiveFilterGroupId(peopleGroupIdCounter.current); }}
                        className="text-[10px] text-gray-400 hover:text-gray-600 ml-auto">Clear all</button>
                    </>
                  )}
                </div>

                {/* Filter groups with AND/OR */}
                {peopleFilterGroups.map((group, gi) => {
                  const groupFilters = peopleFilters.filter(f => group.filterIds.includes(f.id));
                  const isActive = activeFilterGroupId === group.id;
                  return (
                    <div key={group.id}>
                      {gi > 0 && (
                        <div className="flex items-center gap-2 py-1">
                          <div className="flex-1 h-px bg-gray-200" />
                          <button onClick={() => setPeopleFilterGroupOp(prev => prev === 'and' ? 'or' : 'and')}
                            className={`px-2.5 py-0.5 rounded text-[10px] font-bold ${peopleFilterGroupOp === 'or' ? 'bg-amber-100 text-amber-700 border border-amber-300' : 'bg-blue-100 text-blue-700 border border-blue-300'}`}>
                            {peopleFilterGroupOp.toUpperCase()}
                          </button>
                          <div className="flex-1 h-px bg-gray-200" />
                        </div>
                      )}
                      <div className={`border rounded-lg p-2 space-y-1.5 ${isActive ? 'border-violet-300 bg-violet-50/30' : 'border-gray-200'}`}
                        onClick={() => setActiveFilterGroupId(group.id)}>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[9px] text-gray-400 uppercase tracking-wide">Group {gi + 1}</span>
                          {groupFilters.length > 1 && (
                            <button onClick={(e) => { e.stopPropagation(); setPeopleFilterGroups(prev => prev.map(g => g.id === group.id ? { ...g, op: g.op === 'and' ? 'or' : 'and' } : g)); }}
                              className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${group.op === 'or' ? 'bg-amber-50 text-amber-600 border border-amber-200' : 'bg-blue-50 text-blue-600 border border-blue-200'}`}>
                              {group.op.toUpperCase()}
                            </button>
                          )}
                          {peopleFilterGroups.length > 1 && (
                            <button onClick={(e) => { e.stopPropagation(); removePeopleFilterGroup(group.id); }} className="text-gray-300 hover:text-red-500 text-[10px] ml-auto">× Remove group</button>
                          )}
                        </div>
                        {groupFilters.map(f => {
                    const spec = PEOPLE_FILTER_CATALOG.find(s => s.key === f.fieldKey);
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={addPeopleFilterGroup}
                    className="px-3 py-1.5 bg-gray-50 hover:bg-gray-100 text-gray-500 rounded-lg text-xs border border-gray-200">
                    + Add Filter Group
                  </button>
                  {peopleFilters.length > 0 && (
