...
CASE_STUDY_CUSTOMERS: [List 2-3 customers that have published case studies with specific, measurable outcomes. Use the customer's common brand name only (no Inc., Corp., LLC, or all-caps). For each, state the customer name and the key outcome in one line. Only include customers where you found concrete results (metrics, timeframes, or clear before/after). If fewer than 2 case studies with measurable outcomes exist, write "Insufficient case study data."
...
IMPORTANT: Only score changes from the LAST 6 MONTHS. If the most recent significant change is older than 6 months, score 1 regardless of homepage alignment. Older changes are stale and not actionable for a cold email.

Evaluate how much the product has changed recently AND whether the homepage reflects the new direction. Use the CEO's vision and language to describe the "after" state — how they describe where the company is going, not a technical product description. The homepage does not need to match the CEO's visionary tone exactly to score well.

+1: No significant product changes in the last 6 months, OR homepage already reflects recent changes.
+2: Recent change (last 6 months) exists but homepage has some similarities and needs work.
+3: Recent change (last 6 months) and homepage is missing most of the new story.

Return JSON:
{
  "score": 2,
  "changes": [
    {"date": "Month YYYY", "name": "Change name", "before": "What the company story used to be about", "after": "Where the company is heading in the CEO's words", "recency": "X months ago"}
  ],
  "homepage_alignment": "Quote specific homepage copy and assess how well it reflects the new direction.",
  "verdict": "One sentence."
}

Empty array [] for changes if no significant product changes in the last 6 months. Order changes most recent first.`;
...
=== END CEO CONTENT ===`;
  ...
  const processCompany = async (index) => {
    ...
    try {
      ...
      const sortedGaps = Object.entries(scores).sort((a, b) => b[1] - a[1]);
  ...
  const generateForContact = async (contactId, { emailNum = null, e1Factor = '', e2Factor = '' } = {}) => {
    ...
    const buildContextForFactor = (factor) => {
      ...
      if (factor === 'A') {
        ...
        const outcomes = (factorData.outcomes || []).map((o, i) => `${i+1}. ${o}`).join('\n');
        return `<context>\nCompany: ${cleanedCompanyName}\n\nTarget buyer (the person who cares about these outcomes):\n${targetBuyer}\n\nKey outcomes the buyer cares about:\n${outcomes || 'Not available'}\n\nVerdict:\n${verdict}\n\nCase study customers (reference by name if available. If none found, describe outcomes without naming specific customers):\n${cleanCustomerList(fullAccount.caseStudyCustomers) || 'Insufficient case study data'}\n\nTop outcomes from research:\n${fullAccount.top3Outcomes || 'Not available'}\n\nProduct summary:\n${fullAccount.productSummary || 'Not available'}\n\nHomepage hero (first 300 chars):\n${heroText.substring(0, 300)}\n</context>`;
      } else {
    ...
    const getExamplesForFactor = (factor) => {
      ...
      const relevantFactors = new Set([factor, ...(nearbyGroups[factor] || [])].filter(Boolean));
      const matched = trainingExamples.filter(e => e.is_active !== false && relevantFactors.has(e.gap_factor)).slice(0, 6);
    ...
    try {
      ...
      if (campaignEmailConfigs.length > 0) {
        ...
        for (const ec of emailsToGen) {
          ...
          const contextBlock = buildContextForFactor(factor);
          const examplesSection = getExamplesForFactor(factor);
          const factorData = getFactorData(factor);
  ...
  const pushToInstantly = async (instantlyCampaignId) => {
    ...
    try {
      ...
      const allCampaignCts = campaignContacts.map(cc => cc.contacts).filter(Boolean);
      ...
      const maxEmails = Math.max(...leads.map(l => Object.keys(l).filter(k => k.startsWith('email_') && k.endsWith('_subject')).length));
                      ...
                      <div key={ec.id} className="px-5 py-4 border-b border-gray-100">
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={async () => {
                              if (!confirm(`Remove Email ${ec.email_order}?`)) return;
                        ...
                        <div className="space-y-2">
                            ...
                            <div key={comp.id} className="flex items-start gap-2 bg-gray-50 rounded-lg px-3 py-2">
                              <div className="flex flex-col gap-0.5 pt-1">
                                <button onClick={async () => {
                                  ...
                                  const prev = comps[compIdx - 1];
                                  try {
                                    await updateCampaignEmailComponent(comp.id, { component_order: prev.component_order });
                                    await updateCampaignEmailComponent(prev.id, { component_order: comp.component_order });
                                    setCampaignEmailConfigs(configs => configs.map(x => x.id === ec.id ? { ...x, components: x.components.map(c => c.id === comp.id ? { ...c, component_order: prev.component_order } : c.id === prev.id ? { ...c, component_order: comp.component_order } : c) } : x));
                                  } catch {}
                                ...
                                <button onClick={async () => {
                                  ...
                                  const next = comps[compIdx + 1];
                                  try {
                                    await updateCampaignEmailComponent(comp.id, { component_order: next.component_order });
                                    await updateCampaignEmailComponent(next.id, { component_order: comp.component_order });
                                    setCampaignEmailConfigs(configs => configs.map(x => x.id === ec.id ? { ...x, components: x.components.map(c => c.id === comp.id ? { ...c, component_order: next.component_order } : c.id === next.id ? { ...c, component_order: comp.component_order } : c) } : x));
                                  } catch {}
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                  <span className={`text-[9px] px-1 py-0.5 rounded font-medium ${
                                    comp.saved_prompts?.type === 'prompt' ? 'bg-violet-100 text-violet-700' : 'bg-gray-100 text-gray-600'
                                  }`}>{comp.saved_prompts?.type === 'prompt' ? 'prompt' : 'text'}</span>
                                  <select value={comp.prompt_id || ''} onChange={async (e) => {
                                    const promptId = e.target.value;
                                    const sp = savedPrompts.find(p => p.id === promptId);
                                    try {
                                      const updated = await updateCampaignEmailComponent(comp.id, { prompt_id: promptId });
                                      setCampaignEmailConfigs(configs => configs.map(x => x.id === ec.id ? { ...x, components: x.components.map(c => c.id === comp.id ? { ...c, prompt_id: promptId, saved_prompts: sp || updated?.saved_prompts } : c) } : x));
                                    } catch {}
                                  }} className="flex-1 bg-white border border-gray-200 rounded px-1.5 py-0.5 text-[10px] text-gray-600 min-w-0">
                                    <option value="">Select prompt...</option>
                                    {savedPrompts.filter(sp => !comp.saved_prompts?.type || sp.type === comp.saved_prompts?.type || !sp.type || sp.type === 'custom').map(sp => (
                                ...
                                {/* Inline editor */}
                                <details>
                                  <summary className="text-[9px] text-violet-500 cursor-pointer">Edit text</summary>
                                  <textarea id={`comp-textarea-${comp.id}`} value={comp.saved_prompts?.prompt_text || ''} onChange={async (e) => {
                                    const text = e.target.value;
                                    if (comp.saved_prompts?.id) {
                                      setCampaignEmailConfigs(configs => configs.map(x => x.id === ec.id ? { ...x, components: x.components.map(c => c.id === comp.id ? { ...c, saved_prompts: { ...c.saved_prompts, prompt_text: text } } : c) } : x));
                                      try { await updateSavedPrompt(comp.saved_prompts.id, { prompt_text: text }); } catch {}
                                    }
                                  }} rows={comp.saved_prompts?.type === 'prompt' ? 8 : 3} className="w-full mt-1 bg-white border border-gray-200 rounded px-2 py-1 text-[10px] text-gray-600 font-mono leading-relaxed" />
                                  {comp.saved_prompts?.type !== 'prompt' && (
                                    <div className="flex items-center gap-1 mt-1">
                                      ...
                                      {EMAIL_TEMPLATE_VARIABLES.map(v => (
                                        <button key={v.key} onClick={() => {
                                          ...
                                          const current = comp.saved_prompts?.prompt_text || '';
                                          const newText = current.substring(0, start) + `{{${v.key}}}` + current.substring(end);
                                          setCampaignEmailConfigs(configs => configs.map(x => x.id === ec.id ? { ...x, components: x.components.map(c => c.id === comp.id ? { ...c, saved_prompts: { ...c.saved_prompts, prompt_text: newText } } : c) } : x));
                                          if (comp.saved_prompts?.id) {
                                            updateSavedPrompt(comp.saved_prompts.id, { prompt_text: newText }).catch(() => {});
                                          }
                              ...
                              <button onClick={async () => {
                                try {
                                  ...
                                  setCampaignEmailConfigs(configs => configs.map(x => x.id === ec.id ? { ...x, components: x.components.filter(c => c.id !== comp.id) } : x));
                                } catch {}
                          ...
                          <div className="relative">
                            <button onClick={() => {
                              const pickerKey = `compPicker_${ec.id}`;
                              ...
                              <div className="absolute left-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl z-50 min-w-[250px] max-h-[300px] overflow-auto">
                                ...
                                {[{type: 'prompt', label: 'Prompt Component'}, {type: 'text', label: 'Text Component'}].map(({ type, label }) => (
                                  <button key={type} onClick={async () => {
                                    const name = prompt(`Name for new ${label}:`);
                                    if (!name) return;
                                    try {
                                      const sp = await createSavedPrompt({ name, prompt_text: '', type });
