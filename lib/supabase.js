...
export async function removeCampaignContacts(campaignId, contactIds) {
...
// Instantly Activity
export async function getInstantlyActivity() {
  if (!supabase) return [];
  const { data, error } = await supabase
    .from('instantly_activity')
    .select('*');
  if (error) throw error;
  return data || [];
}

export async function upsertInstantlyActivity(rows) {
  if (!supabase || !rows.length) return;
  const { error } = await supabase
    .from('instantly_activity')
    .upsert(rows, { onConflict: 'contact_email' });
  if (error) throw error;
}
